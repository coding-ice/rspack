...
import { LoaderContext, LoaderContextCallback } from "../config/adapterRuleUse";
...
async function loaderImpl({
	...
	const newArgs = await runSyncOrAsync(loaderObject.normal!, loaderContext, args)
...
function worker(workerOptions: WorkerOptions) {
	loaderImpl(workerOptions).then(({ newArgs }) => {
		workerOptions.tx.postMessage({ newArgs });
	})
		.catch((e) => {
			workerOptions.tx.postMessage({ error: e })
		})
}
